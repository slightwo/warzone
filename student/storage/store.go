...
func (s *Store) SaveProfile(profile protocol.UserProfile) error {
	...
	if profile.PasswordHash == "" {
		var oldUser UserRecord
		if err := s.db.Select("password_hash").Where("username = ?", profile.Username).First(&oldUser).Error; err == nil {
			profile.PasswordHash = oldUser.PasswordHash
		}
	}

	dbUser := toDBUser(profile)
	return s.db.Save(&dbUser).Error
}
