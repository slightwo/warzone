...
| 序 | 问题 | 影响 | 成本 | 前置 |
|----|------|------|------|------|
| 3.1 | §2.4 事件系统加重放/持久化 | 🟠 中 | 中-高 | 2.2 稳定后；用于降低 checkpoint 间隔导致的 RPO |
| 3.2 | §3.1 统一 `Version` 语义（或删掉死字段） | 🟡 低 | 中（需想清楚语义） | 2.2 的 Topology/MapEpoch 语义明确后 |
| 3.3 | 动态地图副本组与主权调度：副本补齐、自动提升、旧主回归、可选 K8s 生命周期 | 🟠 中-高 | 很高 | **2.2 的版本化 Topology + MapEpoch fencing 完成后** |
| 3.4 | §1.4 动态分片/负载迁移：热点地图拆 zone/room/shard | 🟠 中 | 很高（demo 阶段可不做） | 3.3 完成；副本扩容不等于写吞吐扩容 |

> 3.3 的目标不是同地图多主写入，而是「单权威写者 + 0..N 个 warm standby」。每张地图任一时刻
> 只有一个 owner 执行 tick/接收写入；副本用于容灾。Lab4 的每地图 Deployment + Leader lease 属于此类
> active-passive 副本组，不应误解为 active-active 扩容。
...
第 2 梯队（严格串行）
   2.1 节点自治 ──> 2.2 拆分控制面 ──> 2.3 统一协议
                      │
第 3 梯队（按需、可独立排期）
   3.3 动态副本组/主权调度 ──> 3.4 动态分片与负载迁移
   3.1 事件重放、3.2 Version 语义可在 2.2 后并行
```
...
> 再动架构（先节点自 tick，后拆控制面，协议统一压轴）→ 最后按 SLA 决定是否投入动态副本组与动态分片。
