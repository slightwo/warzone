...
func main() {
	...
	var stream pb.GatewayService_GameStreamClient
	...
	go func() {
		...
		for {
			pro, err := stream.Recv()
			msg := protocol.FromProtoMessage(pro)
			if err != nil {
	...
	for {
		...
		if key == 3 {
			_ = stream.Send(protocol.ToProtoMessage(protocol.Message{Type: protocol.TypeLogout}))
			return
		}
		...
		addClientNote(note)
		if err := stream.Send(protocol.ToProtoMessage(msg)); err != nil {
...
func auth(addr, mode, username, password, confirm string) (pb.GatewayService_GameStreamClient, *protocol.WorldState, error) {
	...
	msg := protocol.FromProtoMessage(reply)
	if err != nil {
		return nil, nil, fmt.Errorf("接收认证结果失败：%w", err)
	}
	if msg.Type == protocol.TypeError || !msg.OK || msg.State == nil {
		return nil, nil, fmt.Errorf("%s", msg.Error)
	}
	return stream, msg.State, nil
}
