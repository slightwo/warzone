...
1. 完成 Gateway 多副本、真实 readiness、会话租约和 HAProxy 接入。
2. 验证单个 Gateway 故障或 drain 后，新的客户端连接仍能通过负载均衡器进入健康副本。
3. 再按世界性能计划实施轻量限流、最小观测和无变化状态抑制。
4. 仅当压测仍显示明显瓶颈时，再评估 `MapDelta` 或位置索引。
