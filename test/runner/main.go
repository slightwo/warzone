...
func cleanSQLTestData() {
	cmd := exec.Command("psql", "-h", "localhost", "-U", "postgres", "-d", "battleworld", "-c", "DELETE FROM users WHERE username LIKE 'lab3_%'; DELETE FROM hot_sessions WHERE username LIKE 'lab3_%';")
	cmd.Env = append(os.Environ(), "PGPASSWORD=Wu050601&&")
	out, err := cmd.CombinedOutput()
	if err != nil {
		fmt.Fprintf(os.Stderr, "清理 SQL 数据失败: %v, output: %s\n", err, string(out))
	}
}
